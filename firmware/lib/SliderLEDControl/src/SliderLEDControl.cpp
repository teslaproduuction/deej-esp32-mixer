...
void SliderLEDControl::sample() {
    // Median filter over NUM_READINGS samples. Unlike moving average it
    // ignores one-off spikes outright instead of smearing them across the
    // window, which fixes flicker on pots with a flaky wiper contact.
    ...
    averageValue = sorted[NUM_READINGS / 2];
}

void SliderLEDControl::render() {
...
void SliderLEDControl::renderRainbow(uint16_t baseHue) {
    // Each LED gets a hue offset so the strip shows a slice of the rainbow.
    // Caller bumps baseHue every frame to make the rainbow scroll.
    for (int i = 0; i < NUM_LEDS_PER_SLIDER; i++) {
        uint16_t h = baseHue + (uint16_t)(i * (65535 / NUM_LEDS_PER_SLIDER));
        strip.setPixelColor(i, strip.ColorHSV(h));
    }
    strip.show();
}
